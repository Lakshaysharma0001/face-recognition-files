...
# Setting a random
# Replacing TensorFlow model loading and prediction with ONNXRuntime
import onnxruntime as ort
def image_to_embedding(image: np.ndarray, session) -> np.ndarray:
    if image is None or session is None:
        return None
    input_name = session.get_inputs()[0].name
    embedding = session.run(None, {input_name: image[np.newaxis,...]})[0]
