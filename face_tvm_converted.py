...
# Setting a random
# Replacing TensorFlow model with Apache TVM runtime
import tvm
from tvm import relay
from tvm.contrib import graph_executor

# Function to load TVM module
def load_tvm_model(lib_path, graph_json_path, params_path):
...
# Embedding extraction function for TVM
def image_to_embedding(image: np.ndarray, module) -> np.ndarray:
    if image is None or module is None:
        return None
    image = np.expand_dims(image, axis=0).astype('float32')
    module.set_input('input', image)
    module.run()
    embedding = module.get_output(0).asnumpy()[0]
